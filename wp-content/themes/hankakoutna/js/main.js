...
(function() {
  var FormDescription, ImageChanger, ScrollArrows, ScrollView,
    __bind = function(fn, me){ return function(){ return fn.apply(me, arguments); }; };

  ScrollArrows = (function() {

    function ScrollArrows(element) {
      this.arrow_click = __bind(this.arrow_click, this);
      this.main_element = $(element);
      this.element = this.main_element.parent();
      this.make_arrows();
    }

    ScrollArrows.prototype.arrow_click = function(event) {
      this.main_element.trigger("arrow_click", $(event.target).attr("data-direction"));
      return false;
    };

    ScrollArrows.prototype.make_arrows = function() {
      var arrow, direction, _i, _len, _ref, _results;
      _ref = ["left", "right"];
      _results = [];
      for (_i = 0, _len = _ref.length; _i < _len; _i++) {
        ...
        arrow.css("top", this.element.height() / 2 - 37 / 2 + parseInt(this.element.attr("data-arrow-fix")));
        if (direction === "left") {
          arrow.css("left", -37 - (this.element.width() === 940 ? 20 : 10));
          arrow.css("background-position", "0 -37px");
        } else {
          arrow.css("right", -37 - (this.element.width() === 940 ? 20 : 10));
        }
        arrow.click(this.arrow_click);
        arrow.hover(this.mouse_over, this.mouse_out);
        _results.push(this.element.append(arrow));
      }
      return _results;
    };

    ScrollArrows.prototype.mouse_over = function(event) {
      var target;
      target = $(event.target);
      if (target.attr("data-direction") === "left") {
        return target.css("background-position", "-37px -37px");
      } else {
        return target.css("background-position", "-37px 0px");
      }
    };

    ScrollArrows.prototype.mouse_out = function(event) {
      var target;
      target = $(event.target);
      if (target.attr("data-direction") === "left") {
        return target.css("background-position", "0px -37px");
      } else {
        return target.css("background-position", "0px 0px");
      }
    };
  ...
  ScrollView = (function() {
    ...
    function ScrollView(element) {
      this.switch_click = __bind(this.switch_click, this);

      this.arrow_click = __bind(this.arrow_click, this);
      this.element = $(element);
      this.make_scroller();
      this.make_switch();
      this.arrows = new ScrollArrows(this.element);
      this.element.bind("arrow_click", this.arrow_click);
      this.current_page = 0;
    }

    ScrollView.prototype.arrow_click = function(event, direction) {
      if (direction === "left" && this.current_page > 0) {
        return this.goto(this.current_page - 1);
      } else if (direction === "right" && this.current_page < this.max_page_index) {
        return this.goto(this.current_page + 1);
      }
    };

    ScrollView.prototype.switch_click = function(event) {
      var index, target;
      target = $(event.target);
    ...
    ScrollView.prototype.goto = function(index) {
      this.current_page = parseInt(index, 10);
      this.switch_element.find("li").removeClass("selected");
      $(this.switch_element.find("li")[index]).addClass("selected");
    ...
    ScrollView.prototype.make_switch = function() {
      ...
      this.max_page_index = count - 1;
      _results = [];
      for (index = _i = 0, _ref = this.max_page_index; 0 <= _ref ? _i <= _ref : _i >= _ref; index = 0 <= _ref ? ++_i : --_i) {
        link = $("<a>", {
          "data-index": index,
          "href": "#"
        });
  ...
  ImageChanger = (function() {
    ...
    function ImageChanger() {
      ...
      this.prepare_next();
      this.preload();
      this.arrows = new ScrollArrows($("#image-changer div.bottom"));
      $("#image-changer  div.bottom").bind("arrow_click", this.arrow_click);
    }

    ImageChanger.prototype.arrow_click = function(event, direction) {
      var next_step;
      ...
      } else if (direction === "right" && this.current <= 6) {
        next_step = this.current + 1;
        if (next_step > 6) {
          next_step = 1;
        }
      }
      return this.goto(next_step);
    };
  ...
  $(function() {
    ...
    return $(".partners a img").hover(function(event){$(event.target).removeClass("grayscale")}, function(event){$(event.target).addClass("grayscale")});
  });
